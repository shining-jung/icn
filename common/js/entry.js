...
const creatHtml = (data) => {
    ...
    if (item.remark === '도착') {
        ...
        remarkCode = 'step03';
    } else if (item.remark === '착륙') {
        remarkCode = 'step04';
    } else if (item.remark === '회항') {
        remarkCode = 'step06';
    }
    let airlineCode = `${item.flightId.substring(0, 2)}`;
    ...
    return `
        <li class="${remarkCode}">
            <div class="rowItem" data-adate="${item.scheduleDateTime.substring(0, 8)}"  data-terminal="${item.terminalid}" data-scTime="${item.scheduleDateTime.substring(8, 10)}" >
                ...
                <div class="itemAirline ${airlineCode}">
                    <div>
                        <img src="../common/img/${airlineCode}.png"
...
const creatRowItemsSiblingDiv = (data, terminal) => {
    const T1Congestion = Math.trunc(data.t1sumset1);
    const T1CongestionLevelStep = CongestionLevelStep(T1Congestion);
    const T2Congestion = Math.trunc(data.t2sumset1);
    const T2CongestionLevelStep = CongestionLevelStep(T2Congestion);
    let currentDataT1 = `
            <p>[입국장 예상 혼잡도]</p>
            <p class="totalLevel ${T1CongestionLevelStep}">총 예상인원 : <strong>${T1Congestion}명</strong></p>             
            <span>A, B : <strong>${Math.trunc(data.t1sum1)}명</strong> / </span>
            <span>C : <strong>${Math.trunc(data.t1sum3)}명</strong> / </span>
            <span>D : <strong>${Math.trunc(data.t1sum4)}명</strong> / </span>
            <span>E, F : <strong>${Math.trunc(data.t1sum2)}명</strong></span>    
    `;
    let currentDataT2 = `   
            <p>[입국장 예상 혼잡도]</p>
            <p class="totalLevel ${T2CongestionLevelStep}">총 예상인원 : <strong>${T2Congestion}명</strong></p>              
            <span>A : <strong>${Math.trunc(data.t2sum1)}명</strong> / </span>
            <span>B : <strong>${Math.trunc(data.t2sum2)}명</strong></span>        
    `;
    if (terminal == 'P01') {
        return `
            <div>
        ...
        return `
            <div>
                ${currentDataT1}
                <p class="dec">이 항공편은 셔틀트레인을 이용하여 이동하는 탑승동에 도착합니다.</p>      
            </div>    
        `;
